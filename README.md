...
- Покупка товаров
    ...
    4) Проверяется что пользователь авторизован
        ...
        4) 1) Если не авторизован, перенаправляем на страницу авторизации

        4) 2) Если авторизован, продолжаем
    ...
    12)  Сервис `Basket` обрабатывает запрос
        ...
        12) 1) Если товар по каким-то причинам закончился, выводим сообщение пользователю

        12) 2) Если есть в наличии, продолжаем
    ...
    16) Сервис `Payment` отправляет запрос на сервис `ProductCatalog`, зарезервирован ли товар
        ...
        16) 1) если товар не зарезервирован, но есть в наличии - резервируем

        16) 2) если товар не зарезервирован и нет в наличии, нам нужно будет уведомить пользователя

        16) 3) товар зарезервирован
